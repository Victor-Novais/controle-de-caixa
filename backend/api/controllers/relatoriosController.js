...
const generateReport = (req, res) => {
    ...
    const totalValue = compras.reduce((sum, purchase) => sum + purchase.valorCompra * purchase.quantidade, 0);
...
const generateSalesReportByPeriod = (req, res) => {
    ...
    const vendasPorPeriodo = compras.reduce((acc, compra) => {
        const periodo = new Date(compra.dataHoraCompra).toLocaleString('pt-BR', { month: 'long', year: 'numeric' });
        if (!acc[periodo]) {
            acc[periodo] = 0;
        }
        acc[periodo] += compra.valorCompra * compra.quantidade;
        return acc;
    }, {});
